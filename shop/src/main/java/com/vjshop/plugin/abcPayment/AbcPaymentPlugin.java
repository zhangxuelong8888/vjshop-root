...
public class AbcPaymentPlugin extends PaymentPlugin {
	...
	public Map<String, Object> getParameterMap(String sn, String description, HttpServletRequest request) {
		...
		merchant.addElement("MerchantID").setText(pluginConfig.getAttributesMap("merchantId"));
		...
		trxRequest.addElement("MerchantRemarks").setText("vjshop");
	...
	public boolean verifyNotify(PaymentPlugin.NotifyMethod notifyMethod, HttpServletRequest request) {
		try {
			...
			TPaymentLog paymentLog = getPaymentLog(getSn(request));
			if (paymentLog != null && verifySign(message) && pluginConfig.getAttributesMap("merchantId").equals(merchantId) && "0000".equals(returnCode) && paymentLog.getAmount().compareTo(new BigDecimal(amount)) == 0) {
				return true;
			}
			return false;
	...
	private String generateSign(String message) {
		try {
			TPluginConfig pluginConfig = getPluginConfig();
			PrivateKey privateKey = RSAUtils.generatePrivateKey(pluginConfig.getAttributesMap("key"));
