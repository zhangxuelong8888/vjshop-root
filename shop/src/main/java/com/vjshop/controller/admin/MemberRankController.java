...
public class MemberRankController extends BaseController {
	...
	boolean checkName(String previousName, String name) {
		...
		return memberRankService.nameUnique(previousName, name);
	}
	...
	boolean checkAmount(BigDecimal previousAmount, BigDecimal amount) {
		...
		return memberRankService.amountUnique(previousAmount, amount);
	}
	...
	public String save(TMemberRank memberRank, RedirectAttributes redirectAttributes) {
	...
	public String update(TMemberRank memberRank, RedirectAttributes redirectAttributes) {
		if (!isValid(memberRank)) {
			return ERROR_VIEW;
		}
		TMemberRank pMemberRank = memberRankService.find(memberRank.getId());
		if (pMemberRank == null) {
			return ERROR_VIEW;
		}
	...
	public @ResponseBody Message delete(Long[] ids) {
		if (ids != null) {
			for (Long id : ids) {
				long memberCount =  memberService.count(Filter.eq("memberRank", id));
				if (memberCount > 0) {
					TMemberRank memberRank = memberRankService.find(id);
