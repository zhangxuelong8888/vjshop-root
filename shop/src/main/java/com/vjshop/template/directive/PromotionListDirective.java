...
public class PromotionListDirective extends BaseDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		List<Filter> filters = getFilters(params, TPromotion.class);
		List<Order> orders = getOrders(params);
		boolean useCache = useCache(env, params);
		List<TPromotion> promotions = promotionService.findList(memberRankId, productCategoryId, hasBegun, hasEnded, count, filters, orders, useCache);
